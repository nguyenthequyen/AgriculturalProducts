...
namespace AgriculturalProducts.Repository
{
    public class StatisticsAdminRepostitory : BaseRepository<Statistics>, IStatisticsAdminRepostitory
    {
        ...
        {
            var statistics = _applicationContext.Statistics.Where(x => x.Action == (int)Action.Visitor).GroupBy(x => x.CreatedDate.Date).Select(x => new
            {
                label = x.Key,
                y = x.Distinct().Count()
        ...
        {
            var statistics = _applicationContext.Statistics.Where(x => x.Action == (int)Action.Order).GroupBy(x => x.CreatedDate.Date).Select(x => new
            {
                label = x.Key,
                y = x.Distinct().Count()
            }).OrderBy(x => x.label).ToList();
        ...
        {
            var statistics = _applicationContext.Statistics.Where(x => x.Action == (int)Action.CreatedUser).GroupBy(x => x.CreatedDate.Date).Select(x => new
            {
                label = x.Key,
                y = x.Distinct().Count()
            }).OrderBy(x => x.label).ToList();
        ...
        {
            return _applicationContext.Statistics.Where(x => x.Action == (int)Action.Visitor).Count();
        }
    }
}
